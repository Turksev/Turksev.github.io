...
Havuzun tamamı tarandı: 46 paket, 7.848 kelime, **420 bulgu** (172 hata,
...
Önceden şüpheli olarak işaretlenen dokuz başlığın incelemesi tamamlandı; bekleyen kullanıcı
kararı yoktur. PDF bağlamı ve gerektiğinde yetkili sözlüklerle verilen kanonik başlık,
anlam, örnek ve kalıp kararları `tools/kelime-duzeltmeleri.json` içinde tutulur.
`listeyi-aktar.py` bu kaynaktan `data/kelime-aliaslari.js` dosyasını üretir; eski başlık
altındaki Leitner ve test ilerlemesi yeni başlığa kayıpsız birleştirilir. Öğenin hangi sınav,
...
yazıldı: **2.576 kelime + 122 öbek**. Yıldız anlam metnine bağlıdır; `listeyi-aktar.py`
...
## Konu anlatımı kapsam hattı (29.08.2026 tamamlandı)

Konu haritasındaki **129 ünitenin 129 anlatımı da hazırdır**. Altı temel kayıt
`data/konu-metinleri.js`; T07–T61 kayıtları `data/konu-metinleri-t-ek.js`; E01–E68
kayıtları ise `data/konu-metinleri-e1-ek.js` ve `data/konu-metinleri-e2-ek.js` içinde
tutulur. Konu anlatımı için bekleyen DOCX veya üretim paketi yoktur.

Her anlatım değişikliğinden sonra:

```bash
node tools/test-uretim/konu-kapsam-test.js
```

Test temel ve tüm `konu-metinleri-*-ek.js` dosyalarını aynı VM bağlamında yükler; 129/129
kapsamı, tekil kod sahipliğini, başlık/alan eşleşmesini, gerçek kelime sayısını ve ek
anlatımların zorunlu pedagojik bölümlerini doğrular.
...
Cümle üretimi ve doğrulaması Python + headless Chrome ile yürür; konu kapsamı gibi
JavaScript veri regresyonları Node ile çalıştırılır.
