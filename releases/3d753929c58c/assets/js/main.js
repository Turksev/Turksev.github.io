...
(function () {
  ...
  function sayfaIskeleti() {
    ...
    if (ana) {
      ...
      if (!document.querySelector('.skip-link, .atla')) {
    ...
    document.querySelectorAll('.progress').forEach(function (p) {
      ...
      if (!p.hasAttribute('aria-label') && !p.hasAttribute('aria-labelledby')) {
        p.setAttribute('aria-label', 'Çalışma ilerlemesi');
      }
    });

    document.querySelectorAll('table').forEach(function (t) {
      t.querySelectorAll('thead th, tr:first-child th').forEach(function (h) { h.setAttribute('scope', 'col'); });
      if (!t.querySelector('caption') && !t.hasAttribute('aria-label')) {
        var bolum = t.closest('section, article, .card, main');
        var baslik = bolum && bolum.querySelector('h2, h1, h3');
        t.setAttribute('aria-label', baslik ? baslik.textContent.trim() : 'Kural ve örnekler');
      }
      if (t.scrollWidth > t.clientWidth + 2) {
        t.setAttribute('tabindex', '0');
        t.setAttribute('aria-description', 'Tablonun devamı için yatay kaydırın.');
      }
    });
    document.querySelectorAll('.tablo-kaydir, .tablo-kutu').forEach(function (kutu) {
      kutu.setAttribute('tabindex', '0'); kutu.setAttribute('role', 'region');
      if (!kutu.hasAttribute('aria-label')) {
        var tablo = kutu.querySelector('table');
        kutu.setAttribute('aria-label', (tablo && tablo.getAttribute('aria-label')) || 'Yatay kaydırılabilen tablo');
      }
    });
  ...
  function depoCubugu() {
    if (!/[?&]debug=depo(?:&|$)/.test(location.search || '')) return;
    if (!window.DEPO && !document.getElementById('depoVerisi')) {
      var betik = document.createElement('script'); betik.id = 'depoVerisi'; betik.src = '/data/depo.js';
      betik.onload = depoCubugu; document.head.appendChild(betik); return;
    }
    var d = window.DEPO;
  ...
  function depolamaUyarisi() {
    if (!document.body) {
      document.addEventListener('DOMContentLoaded', depolamaUyarisi, { once: true });
      return;
    }
    if (document.getElementById('depolamaUyarisi')) return;
    var uyari = document.createElement('p');
    uyari.id = 'depolamaUyarisi'; uyari.className = 'card';
    uyari.setAttribute('role', 'alert');
    uyari.textContent = 'İlerlemenin bir bölümü veya yerel kopyası kaydedilemedi. Bu sekmeyi kapatmadan Ayarlar’dan yedek indir; tarayıcının depolama alanını kontrol et. Kaydetme tamamlanana kadar ilerleme göstergelerine güvenme.';
    var bag = document.createElement('a'); bag.href = '/ayarlar.html'; bag.textContent = ' Yedekleme ayarlarını aç';
    uyari.appendChild(bag); document.body.insertBefore(uyari, document.body.firstChild);
  }
  ...
  function geriAlKutusu(sonra) {
    ...
    function ciz() {
      ...
      // Son kurtarma yedeği geri alınana veya yenisiyle değiştirilene kadar görünür.
      if (!y) { kutu.hidden = true; return; }
      kutu.hidden = false;
  ...
  window.YDS = {
    ...
    varlikYolu: function (yol) {
      var kok = window.YDS_VARLIK_KOKU || '';
      return /^(assets|data)\//.test(yol) ? kok + yol : yol;
    },
