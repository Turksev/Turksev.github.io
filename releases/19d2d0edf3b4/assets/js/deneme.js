...
(function () {
  ...
  function oturumKaydet() {
    ...
    var kayitli = Oturum.kaydet(test, {
      sira: sira,
      tur: aktifTur,
      form: aktifForm,
    ...
    var bildirim = $('oturumKayitDurumu');
    if (bildirim) bildirim.textContent = kayitli
      ? 'Cevapların bu sekmede otomatik kaydedildi. Yenilersen devam edebilirsin; süre işlemeye devam eder.'
      : 'Oturum kaydedilemedi. Sayfayı kapatırsan bu denemedeki cevapların kaybolabilir.';
    return kayitli;
  }
  ...
  function soruyuGoster() {
    ...
    $('qSecenekler').innerHTML = s.secenekler.map(function (se, i) {
      return '<button class="opt' + (s.cevap === i ? ' secili' : '') + '" type="button" data-i="' + i +
        '" role="radio" tabindex="' + ((s.cevap === null ? i === 0 : s.cevap === i) ? '0' : '-1') +
        '" aria-checked="' + (s.cevap === i ? 'true' : 'false') + '"><span class="key">' + HARF[i] +
  ...
  function cevapla(i) {
    test[sira].cevap = i; oturumKaydet(); soruyuGoster();
    var secili = $('qSecenekler').querySelector('[aria-checked="true"]');
  ...
  $('qSecenekler').setAttribute('role', 'radiogroup');
  $('qSecenekler').setAttribute('aria-labelledby', 'qText');
  $('qSecenekler').addEventListener('keydown', function (e) {
    if (!/^(ArrowRight|ArrowDown|ArrowLeft|ArrowUp|Home|End)$/.test(e.key)) return;
    e.preventDefault(); e.stopPropagation();
    var current = Number(e.target.getAttribute('data-i')) || 0;
    var count = test[sira].secenekler.length;
    var next = e.key === 'Home' ? 0 : (e.key === 'End' ? count - 1 :
      (current + (/Right|Down/.test(e.key) ? 1 : -1) + count) % count);
    cevapla(next);
  });
