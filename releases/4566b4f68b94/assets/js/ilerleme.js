...
(function () {
  ...
  // Kalıcı eski gün anahtarı. UTC takvim sıra numarası değildir; değiştirmek
  // geçmiş kayıtları ve tekrar vadelerini kaydırır. Görünümde gunTarihi kullanılır.
  function tarihGunu(d) {
    if (!d || typeof d.getTime !== 'function' || !isFinite(d.getTime())) return NaN;
    var geceYarisi = new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime();
    return Math.floor(geceYarisi / 86400000);
  }
  function bugun() { return tarihGunu(new Date()); }

  function gunTarihi(g) {
    if (!Number.isSafeInteger(g) || g < 0) return null;
    var taban = new Date(g * 86400000), adaylar = [];
    if (!isFinite(taban.getTime())) return null;
    for (var i = 0; i <= 1; i++) {
      var aday = new Date(taban.getUTCFullYear(), taban.getUTCMonth(), taban.getUTCDate() + i, 12);
      if (tarihGunu(aday) === g) adaylar.push(aday);
    }
    // Bazı eski DST geçişleri iki tarihi aynı anahtara sıkıştırmıştır.
    // Kayıp bilgiyi tahmin ederek yanlış bir güne atama; depo değişmez.
  ...
  function obekTakmaGocu() {
    ...
    if (aday) {
      if (Depo.yaz(K_LEITNER, aday) === false) return 0;
      leitner = aday;
    }
    return tasinan;
  }
  ...
  function kayitYaz(en, kayit, tur) {
    ...
    aday[en] = kayit;
    var ilk = !leitner[en];
    if (ilk && Depo.paketYaz) {
      var p = {}, s = bugunkuSayac();
      p[K_LEITNER] = aday;
      p[K_YENI_SAYAC] = { g: bugun(), n: s.n + 1, ek: s.ek };
      if (Depo.paketYaz(p, 'kart') === false) return false;
      leitneriTazele();
      return true;
    }
    var basarili;
    ...
    if (basarili && ilk && !Depo.paketYaz) return yeniAcildiSay();
    return basarili;
  }
  ...
  /* Yalnizca BASLANMIS kelimeler icin: tekrar gunu geldi mi?
     ...
     gunluk kotayla acilir. Ikisini ayirmazsak ilk gun 7.912 kart cikardi. */
  ...
  function gunlukHedefAyarla(n) {
    return Depo.yaz(K_GUNLUK, Math.max(1, parseInt(n, 10) || GUNLUK_VARSAYILAN)) !== false;
  }
  ...
  function gunlukTavanAyarla(n) {
    return Depo.yaz(K_TAVAN, Math.max(1, parseInt(n, 10) || TAVAN_VARSAYILAN)) !== false;
  }
  ...
  function kotaArtir(n) {
    var s = bugunkuSayac();
    return Depo.yaz(K_YENI_SAYAC, { g: bugun(), n: s.n, ek: s.ek + (parseInt(n, 10) || gunlukHedef()) }) !== false;
  }
  ...
  /* ---------- günlük çalışma sayaçları ----------
     İstatistik sayfası "hız ve ivme"yi buradan hesaplar. Leitner kaydı yalnız
     kartın SON çalışıldığı günü (c) tutar; aynı kart ikinci kez çalışılınca eski
     gün kaybolur, dolayısıyla geçmiş günlerin gerçek yükü ondan çıkarılamaz.
     Bu yüzden günlük toplamlar ayrı ve yalnız artan sayaçlarda tutulur.

       { "20338": {t, y, d, m, z} }
       t: cevaplanan kart (Bildim + İpucuyla + Bilemedim)
       y: o gün ilk kez açılan kart
       d: doğru bilinen kart
       m: 5. kutuya (mezuniyet) ilk kez çıkan kart
       z: "Zaten biliyorum" ile ayıklanan kart — t'ye dahil DEĞİL, yoksa toplu
          ayıklama günü gerçek çalışma hızı gibi görünürdü.

     Cihazlar arası: motorun genel kuralı gereği aynı günü en son yazan cihaz
     kazanır; sürümsüz eski kayıtlar birleşirken alan alan maksimum alınır
     (esitleme-veri.js). İkisinde de toplama yapılmaz — toplamak her yeniden
     eşitlemede aynı çalışmayı yeniden sayardı. Tek cihazda sayılar tamdır. */
  var K_GUNLUK_KAYIT = 'yds-gunluk-kayit';
  var GUNLUK_KAYIT_SINIRI = 800;              // ~2 yıl; fazlası en eskiden budanır
  var GUNLUK_ALANLAR = ['t', 'y', 'd', 'm', 'z'];

  function gunlukKayitlar() {
    var ham = Depo.oku(K_GUNLUK_KAYIT, null);
    var sonuc = {};
    if (!ham || typeof ham !== 'object' || Array.isArray(ham)) return sonuc;
    Object.keys(ham).forEach(function (g) {
      var kayit = ham[g];
      if (!/^\d{1,7}$/.test(g) || !kayit || typeof kayit !== 'object') return;
      var temiz = {};
      GUNLUK_ALANLAR.forEach(function (alan) {
        var v = kayit[alan];
        temiz[alan] = (typeof v === 'number' && isFinite(v) && v > 0) ? Math.floor(v) : 0;
      });
      sonuc[g] = temiz;
    });
    return sonuc;
  ...
  /* Bugünün sayacına ekler. Yazma başarısız olursa (kota dolu, gizli mod)
     sessizce geçilir: istatistik kaydı hiçbir zaman çalışmayı engellememeli. */
  function gunlukIsle(ekler) {
    var g = String(bugun());
    var kayitlar = gunlukKayitlar();
    var mevcut = kayitlar[g] || {};
    var yeni = {};
    GUNLUK_ALANLAR.forEach(function (alan) {
      yeni[alan] = (mevcut[alan] || 0) + (ekler[alan] || 0);
    });
    if (Depo.kayitlariYaz) {
      var degisiklik = {};
      degisiklik[g] = yeni;
      Depo.kayitlariYaz(K_GUNLUK_KAYIT, degisiklik);
    } else {
      kayitlar[g] = yeni;
      Depo.yaz(K_GUNLUK_KAYIT, kayitlar);
    }
    gunlukBuda(kayitlar, g);
  }

  function gunlukBuda(kayitlar, bugunku) {
    var gunler = Object.keys(kayitlar);
    if (gunler.length <= GUNLUK_KAYIT_SINIRI || !Depo.kayitlariSil) return;
    gunler.sort(function (a, b) { return Number(a) - Number(b); });
    var silinecek = gunler.slice(0, gunler.length - GUNLUK_KAYIT_SINIRI)
      .filter(function (g) { return g !== bugunku; });
    if (silinecek.length) Depo.kayitlariSil(K_GUNLUK_KAYIT, silinecek);
  }
  ...
  function zatenBiliyorum(en, tur) {
    ...
    gunlukIsle({ z: 1, y: ilk ? 1 : 0, m: onceki < EN_UST_KUTU ? 1 : 0 });
    return EN_UST_KUTU;
  }
  ...
  function dogru(en, tur) {
    ...
    gunlukIsle({ t: 1, d: 1, y: ilk ? 1 : 0,
      m: (k >= EN_UST_KUTU && onceki < EN_UST_KUTU) ? 1 : 0 });
    return k;
  }
  ...
  var YEDEK_ALANLARI = [
    ...
    'yds-test-yanlis', 'yds-rekor', 'yds-yeni-sayac', 'yds-gunluk-kayit'
  ];
  ...
  function yanlisEkle(kayit) {
    ...
    return Depo.yaz(K_YANLIS, defter) !== false;
  }
  ...
  function yanlisCoz(kayit) {
    var defter = Depo.oku(K_YANLIS, []);
    if (!Array.isArray(defter)) return true;
    var anahtar = yanlisAnahtar(kayit);
    var mevcut = defter.filter(function (y) { return y.a === anahtar; })[0];
    if (!mevcut || mevcut.sd === bugun()) return true;
    mevcut.c = (mevcut.c || 0) + 1;
    mevcut.sd = bugun();
    mevcut.u = Date.now();
    if (mevcut.c >= 2) defter = defter.filter(function (y) { return y.a !== anahtar; });
    return (defter.length ? Depo.yaz(K_YANLIS, defter) : Depo.sil(K_YANLIS)) !== false;
  }
  ...
  function yanlisTemizle() { return Depo.sil(K_YANLIS) !== false; }
  ...
  function kategoriKaydet(kat, dogruMu, soruId) {
    ...
    return Depo.yaz(K_KATEGORI, s) !== false;
  }
  ...
  function kategoriSifirla() { return Depo.sil(K_KATEGORI) !== false; }
  ...
  function sonucEkle(kayit) {
    ...
    return Depo.yaz(K_GECMIS, g) !== false;
  }
  ...
  function gecmisSifirla() { return Depo.sil(K_GECMIS) !== false; }
  ...
  function testYanlis(en, tur) {
    ...
    var d = testDefteri(), k = d[en] || { n: 0 };
    k.n = (k.n || 0) + 1; k.t = Date.now(); k.u = k.t; k.c = 0;
    delete k.sd; d[en] = k;
    if (!Depo.paketYaz) {
      if (Depo.yaz(K_TEST_YANLIS, d) === false) return false;
      return yanlis(ad, tur) !== false;
    }
    ...
    var p = {};
    p[K_TEST_YANLIS] = d; p[K_LEITNER] = aday;
    if (ilk) {
      var s = bugunkuSayac();
      p[K_YENI_SAYAC] = { g: bugun(), n: s.n + 1, ek: s.ek };
    }
    if (Depo.paketYaz(p, 'test-yanlis') === false) return false;
    leitneriTazele();
    return true;
  }
  ...
  function testDogru(en, tur) {
    ...
    var d = testDefteri();
    if (!d[en]) return true;
    if (d[en].sd === bugun()) return true;
    d[en].c = (d[en].c || 0) + 1;
    d[en].sd = bugun();
    d[en].u = Date.now();
    if (d[en].c >= 2) delete d[en];
    return (Object.keys(d).length ? Depo.yaz(K_TEST_YANLIS, d) : Depo.sil(K_TEST_YANLIS)) !== false;
  }
  ...
  function testTemizle() { return Depo.sil(K_TEST_YANLIS) !== false; }
  ...
  function konuYaz(kod, alanlar) {
    ...
    if (Depo.yaz(K_KONU, hepsi) === false) return false;
    return mevcut;
  }

  function konuSifirla() { return Depo.sil(K_KONU) !== false; }
