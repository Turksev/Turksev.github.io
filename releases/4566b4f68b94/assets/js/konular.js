...
(function () {
  ...
  var acikKategori = Depo.oku('yds-konu-kategoriler', {});
  if (!acikKategori || typeof acikKategori !== 'object') acikKategori = {};
  var sonKonuKod = '';
  ...
  function ciz() {
    ...
    elListe.innerHTML = sira.map(function (kat, katIndex) {
      ...
      var kayitAnahtari = eksenIndex + ':' + kat;
      var filtreVar = !!(elAra.value.trim() || elKatman.value || elEtki.value || elDurum.value);
      var acikMi = Object.prototype.hasOwnProperty.call(acikKategori, kayitAnahtari)
        ? acikKategori[kayitAnahtari] : (filtreVar || katIndex === 0);
      ...
      panel.querySelector('.tani-ac').addEventListener('click', function () {
        ...
        oylar.innerHTML = '<p class="small muted">Her maddeyi dürüstçe değerlendir:</p>' +
          Array.apply(null, { length: soruSayisi }).map(function (_, i) {
            ...
            return '<div class="tani-oy" data-i="' + i + '" role="radiogroup" aria-labelledby="' + etiket + '">' +
              '<b id="' + etiket + '">' + (i + 1) + '. madde</b>' +
              '<button type="button" role="radio" data-p="1" aria-checked="false" tabindex="0">Buldum</button>' +
              '<button type="button" role="radio" data-p="0.5" aria-checked="false" tabindex="-1">Kısmen</button>' +
              '<button type="button" role="radio" data-p="0" aria-checked="false" tabindex="-1">Bilemedim</button></div>';
        ...
        oylar.addEventListener('click', function (e) {
          ...
          satir.querySelectorAll('button').forEach(function (x) {
            ...
            x.setAttribute('aria-checked', secili ? 'true' : 'false');
            x.tabIndex = secili ? 0 : -1;
          });
          oylar.querySelector('.tani-kaydet').disabled = Object.keys(puanlar).length !== soruSayisi;
        });
        oylar.addEventListener('keydown', function (e) {
          var b = e.target.closest('button[data-p]');
          if (!b || !/^(ArrowRight|ArrowDown|ArrowLeft|ArrowUp|Home|End)$/.test(e.key)) return;
          e.preventDefault(); e.stopPropagation();
          var buttons = Array.prototype.slice.call(b.closest('.tani-oy').querySelectorAll('button'));
          var index = e.key === 'Home' ? 0 : e.key === 'End' ? buttons.length - 1 :
            (buttons.indexOf(b) + (/Right|Down/.test(e.key) ? 1 : -1) + buttons.length) % buttons.length;
  ...
  function haritayaDon() {
    var son = null;
    EKSENLER.some(function (eksen, i) {
      son = eksen.u.filter(function (u) { return u.k === sonKonuKod; })[0];
      if (!son) return false;
      eksenIndex = i;
      return true;
    });
    if (son) {
      acikKategori[eksenIndex + ':' + son.kat] = true;
      Depo.yaz('yds-konu-kategoriler', acikKategori);
      Depo.yaz(EKSEN_ANAHTAR, eksenIndex);
      if (!suzulmusUniteler().some(function (u) { return u.k === sonKonuKod; })) {
    ...
    var hedef = geriOdak && (geriOdak.querySelector('.unite-ac') || geriOdak.querySelector('a'));
    (hedef || elAra).focus();
    window.scrollTo({ top: 0, behavior: window.YDS.hareket() });
  }
  ...
  elListe.addEventListener('click', function (e) {
    ...
    if (katBas) {
      ...
      acikKategori[eksenIndex + ':' + kat] = katBas.getAttribute('aria-expanded') !== 'true';
      Depo.yaz('yds-konu-kategoriler', acikKategori);
      ciz();
