...
(function () {
  ...
  function create(raw, cards, today) {
    var days = Object.create(null), traces = Object.create(null), first = null;
    Object.keys(raw || {}).forEach(function (key) {
      var g = Number(key), row = raw[key];
      if (!/^\d+$/.test(key) || !Number.isSafeInteger(g) || g < 0 || g > today || !row || typeof row !== 'object' || Array.isArray(row)) return;
      days[g] = empty();
    ...
    Object.keys(cards || {}).forEach(function (id) {
      ...
      if (!Number.isSafeInteger(g) || g < 0 || g > today || (first !== null && g >= first)) return;
      traces[g] = (traces[g] || 0) + 1;
    });
    return {days:days,traces:traces,first:first,today:today};
  }
  ...
  function observed(m, start, end) {
    return m.first === null ? 0 : Math.max(0, Math.min(end,m.today) - Math.max(start,m.first) + 1);
  }
  ...
  function period(m, length) {
    ...
    var previous = sum(m,start-length,start-1), complete = observed(m,start-length,start-1) === length;
  ...
  function streak(m) {
    function active(g) { var r=day(m,g); return r.t+r.z>0; }
  ...
  function weeks(m) {
    ...
    var comparable=observed(m,earlier,earlier+elapsed-1)===elapsed;
  ...
  YDS.IstatistikHesap={create:create,day:day,sum:sum,observed:observed,average:average,period:period,streak:streak,weeks:weeks,weekday:weekday,plan:plan};
})();
