...
(function () {
  ...
  var UYGULAMA_SURUMU = window.YDS_SURUM || 'yerel-gelistirme';
  ...
  var ALAN_ADLARI = {
    ...
    'yds-gunluk-kayit': 'Günlük çalışma sayaçları',
  ...
  /* Günlük çalışma sayacı: anahtar gün numarası, değer {t,y,d,m,z} sayaçları.
     Alanların hepsi isteğe bağlı; eksik olan sıfır sayılır (ilerleme.js de öyle okur). */
  function gunlukKaydi(v, ad) {
    alanlariDogrula(v, ['t', 'y', 'd', 'm', 'z'], [], ad);
    var r = {};
    ['t', 'y', 'd', 'm', 'z'].forEach(function (alan) {
      r[alan] = kendi(v, alan) ? tamSayi(v[alan], 0, 1000000, ad + ' “' + alan + '” sayacı') : 0;
    });
    return r;
  }

  function gunKimligi(id) {
    if (!/^\d{1,7}$/.test(String(id))) {
      throw new Error(ALAN_ADLARI['yds-gunluk-kayit'] + ' gün kimliği bir gün numarası olmalı: ' + id);
    }
  ...
  function alaniDogrula(alan, v) {
    ...
    if (alan === 'yds-gunluk-kayit') {
      if (sadeNesne(v) && Object.keys(v).length > 5000) {
        throw new Error(ALAN_ADLARI[alan] + ' en çok 5.000 gün içerebilir.');
      }
      return haritaDogrula(v, alan, gunlukKaydi, gunKimligi);
    }
  ...
  $('yerelSil').addEventListener('click', function () {
    ...
    durum('Sekiz yerel ilerleme alanı sıfırlandı; dört çalışma tercihi korundu. Son kurtarma yedeği geri alınana veya yeni bir yedekle değiştirilene kadar kullanılabilir.', 'ok');
  });
