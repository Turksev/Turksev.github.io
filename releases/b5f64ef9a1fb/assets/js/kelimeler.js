...
(function () {
  ...
  function katmanlariCiz() {
    ...
    $('katmanlar').innerHTML = Veri.KATMANLAR.filter(function (k) {
      ...
      // Puan aralığı hem baloncukta (data-puan, fare/klavye) hem de title'da
      // durur; dokunmatik cihazda uzun basış aynı metni gösterir.
      var puan = Veri.katmanPuanMetni(k);
      return '<button type="button" class="katman' + (acik ? ' acik' : '') + '" data-k="' + k + '"' +
        ' data-puan="' + kacar(puan) + '"' +
        ' title="' + kacar(puan + ' — ' + Veri.KATMAN_ACIKLAMA[k]) + '" aria-pressed="' + (acik ? 'true' : 'false') + '">' +
  ...
  function kartCiz() {
    ...
    // Bir gösterimde ipucu, cevap ve yeniden çizimler aynı örneği kullanır.
    // Sadece yeni karta geçişte (tek kartlı destede de) yeni örnek seçilir.
    if (kartOrnekKelime !== d.e || kartOrnekleri.length !== tam.a.length) {
      kartOrnekKelime = d.e;
      kartAcik = false;
      ipucuAcik = false;
      kartOrnekleri = Veri.ornekleriSec ? Veri.ornekleriSec(d.e, tam.a) : tam.a;
    }
    ...
    $('kartOrnek').innerHTML = kaliplar(tam) +
      kartOrnekleri.filter(function (a) { return a.ex; }).map(function (a) {
        return (a.toplam > 1 ? '<span class="muted small">Çalışma örneği ' + a.sira + ' / ' + a.toplam + '</span><br>' : '') +
          '<span lang="en">' + kacar(a.ex) + '</span><br><span style="opacity:.8">' +
          kacar(a.exTr) + '</span>';
    ...
    var ilkOrnek = kartOrnekleri[0] || {};
    var bosluklu = ilkOrnek.ex ? bosluklaCumle(d.e, ilkOrnek.ex) : null;
    ...
    $('kartIpucu').textContent = kartAcik
      ...
      : 'Cevabı açmak için karta tıkla · istersen cevabı görmeden işaretle';
    ...
    elKart.setAttribute('aria-label', d.e + (kartAcik ? ' kelimesinin cevabı açık' : ' kelimesinin cevabını göster'));
  ...
  function kartAc(e) {
    if (e && e.target && e.target !== elKart &&
        e.target.closest('button, a, input, select, textarea, [contenteditable="true"]')) return;
    if (kartAcik || window.YDS.metinSecildi(elKart)) return;
    kartAcik = true;
    kartCiz();
  }
  elKart.addEventListener('click', kartAc);
  elKart.addEventListener('keydown', function (e) {
    if (e.defaultPrevented || e.repeat || e.isComposing) return;
    if (e.target === elKart && e.key === 'Enter') { e.preventDefault(); kartAc(); }
  });
