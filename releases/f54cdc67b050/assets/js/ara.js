  ...
  function ara(q) {
    ...
    if (!tur || tur === 'kelime') {
      ...
      var kelimeler = Veri.dizin.filter(function (d) {
        ...
        return eslesmePuani(a.e, a.t, a.y, q) - eslesmePuani(b.e, b.t, b.y, q);
      });
      gruplar.push({
    ...
    if (!tur || tur === 'obek') {
      var obekler = (window.OBEKLER || []).map(function (o) {
        return { o: o, p: eslesmePuani(o.f, o.y + ' ' + o.a.map(function (a) { return a.tr; }).join(' '),
          o.a.map(function (a) { return a.ex + ' ' + a.exTr; }).join(' '), q) };
      }).filter(function (x) { return x.p < 9; }).sort(function (a, b) { return a.p - b.p; });
      var asilObekler = obekler.filter(function (x) { return x.p < 4; });
      var ornekObekler = obekler.filter(function (x) { return x.p === 4; });
      [asilObekler, ornekObekler].forEach(function (liste, grupIndex) {
      gruplar.push({
        ad: grupIndex ? 'Öbek örnek cümlelerinde eşleşenler' : 'Öbekler',
        kapali: grupIndex === 1, url: 'obekler.html', n: liste.length,
        satirlar: liste.slice(0, SINIR).map(function (x) {
          var o = x.o;
          return {
            bas: vurgula(o.f, q),
      ...
      gruplar.push({
        ...
        satirlar: sorular.slice(0, SINIR).map(function (s, i) {
          return {
            ...
            rozet: s.kat,
            tip: 'soru', anahtar: String((window.SORULAR || []).indexOf(s))
          };
        })
      });
    }
  ...
  function ciz() {
    ...
    elSonuc.innerHTML = gruplar.map(function (g) {
      var fazla = g.n > g.satirlar.length
        ...
        : '';
      return (g.kapali ? '<details class="arama-ornekler"><summary>' + g.ad + ' (' + g.n + ')</summary>' : '') +
        '<h2 style="margin-top:28px">' + g.ad +
        ...
        }).join('') + fazla + '</div>' + (g.kapali ? '</details>' : '');
    }).join('');
  }
    ...
    if (tip === 'kelime') {
      ...
      if (k && !Veri.katmanYukluMu(k)) {
        Veri.katmanYukle(k).then(function () {
          if (istek !== kartIstegi || !$('kartPerde').open) return;
